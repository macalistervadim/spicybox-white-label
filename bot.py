...
Optional env:
  ...
  DEV_MODE=1         — кнопки $2–$50 сразу зачисляют баланс без реальной оплаты (для теста КП)
"""
...
WELCOME_TEXT = (
    "👋 Добро пожаловать! Спасибо что заглянул!\n\n"
    "Наш бот — это агрегатор нейросетей, который создан для тех, кто хочет "
    "использовать лучшие нейросети в одном месте и не хочет оплачивать "
    "ежемесячные подписки.\n\n"
    "Здесь вы можете создавать, улучшать и редактировать изображения или видео — "
...
PHOTO_MENU_TEXT = (
    "🌄 Выберите тип нейросетей для работы с фотографиями и изображениями:\n\n"
...
PHOTO_CREATE_TEXT = (
    "Выберите генератор изображений с которым хотите работать.\n\n"
    "❓ Если вы не знаете какой выбрать:\n\n"
    ...
    "⚠️ <b>Внимание!</b> Почти все генераторы используют автоматическую модерацию "
...
PHOTO_CREATE_ACTIVE = (
    "Генератор изображений активирован. Можете отправлять задания на создание "
    "изображений.\n\n"
...
PHOTO_ENHANCE_MENU = (
    "Выберите инструмент для улучшения фото или изображения, который вам нужен, "
    "и после его активации все отправляемые фото будут обработаны этим инструментом."
)

PHOTO_ENHANCE_ACTIVE = (
    "Выбранный способ работы с фотографиями активирован. Теперь вы можете загружать "
    "фотографии в форматах jpeg, jpg, png, heic и размером не более 20 мб."
)

PHOTO_MODIFY_MENU = (
    "Выберите способ изменения фотографий и изображений, который вам нужен, "
    "и после его активации все отправляемые изображения будут обработаны этим способом."
)
...
VIDEO_MENU_TEXT = (
    "🎬 Выберите тип нейросети, с которой хотите работать. Мы предлагаем все "
...
VIDEO_IMG2VID_MENU = (
    "🎬 Выберите генератор видео нажав на одну из кнопок в меню ниже.\n\n"
    "🖼️ <b>Видео из изображений</b> — этот тип генераторов создаёт видео из "
...
VIDEO_IMG2VID_ACTIVE = (
    "Режим создания видео активирован. Для создания видео загрузите изображение "
...
VIDEO_TXT2VID_MENU = (
    "🎬 Выберите генератор видео, с которым хотите работать.\n\n"
    "📝 <b>Видео из текста (Text-to-Video)</b> — этот тип генераторов создаёт "
...
VIDEO_TXT2VID_ACTIVE = (
    "Режим создания видео из текста активирован. Для создания видео вам нужно "
    "отправить сообщение с текстом, в котором будет подробно описано какое видео "
    "вы хотите получить."
)

VIDEO_UPSCALE_MENU = (
    "🎬 Выберите инструмент для улучшения видео, с которым хотите работать."
)

VIDEO_UPSCALE_ACTIVE = (
    "Режим улучшения видео активирован. Загрузите видео, у которого вы хотите "
    "улучшить качество."
)
...
def low_balance_text(user_id: int) -> str:
    return (
        f"На вашем счете: {format_balance(user_id)}\n"
...
async def pay_package(callback: CallbackQuery, bot: Bot) -> None:
    ...
    usd_decimal = Decimal(usd)

    if DEV_MODE:
        new_balance = add_balance(callback.from_user.id, usd_decimal)
        await callback.message.answer(
            f"🧪 <b>DEV:</b> баланс пополнен на <b>${usd_decimal:.2f}</b>\n"
    ...
    await bot.send_invoice(
        ...
        payload=f"topup:{callback.from_user.id}:{usd}",
        provider_token="",
        currency="XTR",
...
async def main() -> None:
    ...
    logger.info("Spark AI Creator bot starting (STARS_PER_USD=%s, DEV_MODE=%s)", STARS_PER_USD, DEV_MODE)
